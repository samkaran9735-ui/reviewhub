    ...
    <div style={{ minHeight: '100vh', background: '#f8f8f6' }}>
      ...
      <nav style={{ background: '#1a1a1a', padding: '14px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
          ...
          <span style={{ fontSize: '13px', color: '#aaa' }}>Welcome, Admin</span>
        </div>
      </nav>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                      <thead>
                        <tr style={{ background: '#f8f8f6' }}>
                          ...
                          <th style={{ textAlign: 'left', padding: '12px 16px', color: '#666', fontWeight: '500' }}>Reviews</th>
                      ...
                      <tbody>
                        {products.map(p => (
                          <tr key={p.id} style={{ borderTop: '1px solid #f0f0f0' }}>
                            ...
                            <td style={{ padding: '12px 16px', color: '#1a1a1a' }}>₹{p.price.toLocaleString('en-IN')}</td>
                            <td style={{ padding: '12px 16px', color: '#666' }}>{p.reviews_count.toLocaleString()}</td>
                            <td style={{ padding: '12px 16px' }}>
                          ...
                          <div key={r.id} style={{ background: '#fff', border: '1px solid #eee', borderRadius: '12px', padding: '16px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                              <div>
                                ...
                                <div style={{ fontSize: '12px', color: '#666' }}>by {r.reviewer_name} · {r.products?.name} · Score: {r.score}/10 · {'★'.repeat(r.star_rating)}</div>
                              </div>
                              <div style={{ display: 'flex', gap: '6px' }}>
                ...
                <div style={{ maxWidth: '600px' }}>
                  ...
                  <div style={{ background: '#fff', border: '1px solid #eee', borderRadius: '16px', padding: '24px' }}>
                    ...
                    <div style={{ marginBottom: '16px' }}>
                      ...
                      <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder="Short description of the product..."
                    ...
                    <div style={{ display: 'flex', gap: '10px' }}>
                      <button onClick={() => setTab('products')} style={{ padding: '10px 20px', border: '1px solid #ddd', borderRadius: '8px', background: '#fff', fontSize: '14px', cursor: 'pointer' }}>
                        Cancel
                      </button>
